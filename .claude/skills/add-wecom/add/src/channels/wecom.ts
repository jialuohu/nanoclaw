...
export class WeComChannel implements Channel {
  ...
  async ensureAccessToken(): Promise<string> {
    ...
    if (!data.access_token) {
      throw new Error(`Failed to get WeCom access token: ${data.errmsg || 'unknown error'}`);
    }

    this.accessToken = data.access_token;
    // Buffer 300 seconds before actual expiry
    this.accessTokenExpiry = Date.now() + ((data.expires_in || 7200) - 300) * 1000;
    return this.accessToken;
  }
...
registerChannel('wecom', (opts: ChannelOpts) => {
  ...
  const port = parseInt(
    process.env.WECOM_PORT || env.WECOM_PORT || '9800',
    10,
  );
